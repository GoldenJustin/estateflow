...
docker build \
  ...
  --tag=golden-erpnext:16-estateflow-0.1.4 \
...
CUSTOM_TAG=16-estateflow-0.1.4
PULL_POLICY=never
```
...
curl -sSI "$BASE_URL/assets/estateflow/css/estateflow.css?v=0.1.4" | grep -Ei 'HTTP/|content-type'
curl -sSI "$BASE_URL/assets/estateflow/css/estateflow-guide.css?v=0.1.4" | grep -Ei 'HTTP/|content-type'
```
