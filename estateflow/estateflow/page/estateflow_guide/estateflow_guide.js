...
class EstateFlowGuide {
    constructor(wrapper) {
        ...
        this.page.add_inner_button(__("Client Test Center"), () => frappe.set_route("estateflow-test-center"));
        ...
        this.content(`
            <section class="ef-guide-grid two">
                <article class="ef-guide-card ef-setup-explainer">
                    ...
                    <ul><li>${__("EstateFlow Settings business-mode switches")}</li><li>${__("Default company, country, currency and cost centre")}</li><li>${__("Eight service Items for rent, accommodation, utilities, sales and commission")}</li><li>${__("Seven editable Email Templates for activation, expiry, invoice, payment, overdue and reservation messages")}</li><li>${__("One initial portfolio when selected")}</li><li>${__("Mode-aware guide and Command Center shortcuts")}</li></ul>
    ...
    render_help() {
        const questions = [[__("I ran setup but the dashboard is still zero"),__("Setup creates safe masters and configuration, not demonstration transactions. Create Portfolio → Property → Space → Customer → agreement/booking/sale. Dashboard numbers are live totals.")],[__("What is a Portfolio?"),__("A Portfolio is the reporting and management group above properties: one owner, housing program, hotel group, development, branch or investment fund. It lets you filter the Command Center and Portfolio Performance without changing accounting companies.")],[__("What is a Property versus a Space?"),__("Property is the location/business asset; Space is the inventory being rented, booked, sold or allocated. A hotel is a Property and each room is a Space.")],[__("Why was no automatic invoice created?"),__("Submit the agreement, enable Automatically Generate Invoices, choose Invoice Trigger, add a valid recurring Item/charge, and ensure the trigger date is reached. On Activation bills immediately when configured; On Period Start bills on the period date; Days Before bills by lead days. Use Generate Due Invoices and Contract Billing Tracker to verify.")],[__("Where did my manual invoice go?"),__("Set the EstateFlow Agreement/Property fields on the Sales Invoice before submission. The submit event creates a Contract Billing Event. Use Sync Existing Invoice Links to rebuild tracking for older invoices.")],[__("Where are public property listings?"),__("Activate the Property Listing, enable Publish on Public Property Portal and add a Featured Image. View all at /properties or use View Public Listing on the document.")],[__("How do email alerts work?"),__("Configure an outgoing Email Account, enable EstateFlow Email Notifications, choose reminder days and Email Templates. Every queued event is deduplicated in EstateFlow Notification Log.")],[__("Why can I not book or lease a unit?"),__("The Space must be Ready, enabled for the intended use and free of overlapping reservations/agreements. Sold, blocked and owner-occupied spaces cannot be booked.")],[__("Why is a room available but dirty?"),__("Occupancy and housekeeping are separate. Checkout can release occupancy while marking the room Dirty. Complete a Housekeeping Work Order before the next guest.")],[__("Why do I still see modules I did not enable?"),__("Business modes focus setup and recommendations; they do not delete or lock records. This permits one company to add another real-estate business without reinstalling the app.")],[__("Where are payments and the general ledger?"),__("EstateFlow uses standard ERPNext Sales Invoice, Purchase Invoice, Payment Entry and Journal Entry plus Contract Billing Event for operational tracking.")],[__("How do I test a client deployment?"),__("Open EstateFlow Test Center, choose a business case, create a UAT Run, record actual result/evidence for each test and reconcile all reports before client sign-off.")],[__("Can setup be run more than once?"),__("Yes. It updates settings and creates only missing standard Items, Email Templates and portfolio. Existing property and transaction records are not deleted.")]];
